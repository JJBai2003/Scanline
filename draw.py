...
def scanline_convert(polygons, i, screen, zbuffer ):
    ...
    swap = False
    p = [[polygons[i][1], 0], [polygons[i + 1][1], 1], [polygons[i + 2][1], 2]]
    p.sort()
    b = polygons[i + p[0][1]]
    m = polygons[i + p[1][1]]
    t = polygons[i + p[2][1]]
    ...
    dx0 = (t[0] - b[0]) / (1 + t[1] - b[1])
    dx1 = (m[0] - b[0]) / (1 + m[1] - b[1])
    dx1_1 = (t[0] - m[0]) / (1 + t[1] - m[1])
    dz0 = (t[2] - b[2]) / (1 + t[1] - b[1])
    dz1 = (m[2] - b[2]) / (1 + m[1] - b[1])
    dz1_1 = (t[2] - m[2]) / (1 + t[1] - m[1])

    while (y0 <= t[1]):
        if m[1] == b[1] and not swap:
            dx1 = dx1_1
            x1 = m[0]
            dz1 = dz1_1
            z1 = m[2]
...
def draw_line( x0, y0, z0, x1, y1, z1, screen, zbuffer, color ):
    ...
    dz = (z1 - z0) / (1 + loop_end - loop_start)
